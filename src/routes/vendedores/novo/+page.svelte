<script lang="ts">
    ...
    async function save() {
        ...
        try {
            ...
            const res = await db.execute(
                `INSERT INTO vendedores (${q.columns}) VALUES (${q.placeholders})`,
                q.values,
            );

            // Get the inserted ID (assuming db.execute returns info or we can fetch it)
            // In many SQLite wrappers, we can get last_insert_rowid()
            const sellerIdRes = await db.select(
                "SELECT last_insert_rowid() as id",
                [],
            );
            const sellerId = (sellerIdRes as any)[0].id;

            // 2. Insert Praças Relationships
            if (selectedPracas.length > 0) {
                for (const pracaId of selectedPracas) {
                    await db.execute(
                        "INSERT INTO vendedor_pracas (vendedor_id, praca_id) VALUES ($1, $2)",
                        [sellerId, parseInt(pracaId)],
                    );
                }
            }
            ...
            toast.error(e.message || "Erro ao salvar vendedor");
            console.error(e);
        }
    }
    ...
    async function loadAuxiliaryData() {
        try {
            const pracasRes = await db.select(
                "SELECT * FROM pracas ORDER BY nome",
                [],
            );
...
<div class="p-6 w-full mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-3xl font-bold tracking-tight">Novo Vendedor</h1>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>Status e áreas de atuação.</Card.Description>
            </Card.Header>
            <Card.Content class="space-y-4">
                <div class="grid gap-2">
                    <Label for="ativo">Status</Label>
                    <Select.Root type="single" bind:value={sellerData.ativo}>
