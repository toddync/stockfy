          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Content>
              <DropdownMenu.Group class="*:[&>svg]:ml-auto">
                ...
                <DropdownMenu.Item
                  class={(v == "emitido" && "bg-accent/50") || ""}
                >
                  Emitido
                  <ClockFading class="stroke-amber-400" />
                </DropdownMenu.Item>
                <DropdownMenu.Item
                  class={(v == "vendido" && "bg-accent/50") || ""}
                >
                  Vendido
                  <BadgeCheck class="stroke-lime-400" />
                </DropdownMenu.Item>
                <DropdownMenu.Item
                  class={(v == "devolvido" && "bg-accent/50") || ""}
                >
                  Devolvido
                  <X class="stroke-blue-400" />
                </DropdownMenu.Item>
                <DropdownMenu.Item
                  class={(v == "retornado" && "bg-accent/50") || ""}
                >
                  Retornado
                  <X class="stroke-orange-400" />
                </DropdownMenu.Item>
                <DropdownMenu.Item
                  class={(v == "cancelado" && "bg-accent/50") || ""}
                >
                  Cancelado
                  <Ban class="stroke-red-500" />
                </DropdownMenu.Item>
            ...
            <Table.Body>
              {#each order.itens as item (item.id)}
                <Table.Row>
                  ...
                  <Table.Cell class="text-end">
                    R$ {(item.quantidade_saida - item.quantidade_retorno) *
                      item.preco_venda}
                  </Table.Cell>
                </Table.Row>
              {/each}
