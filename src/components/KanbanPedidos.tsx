...
interface Pedido {
  ...
  data_pedido: string;
  valor_total: number;
  situacao: 'orcamento' | 'confirmado' | 'separacao' | 'entregue' | 'cancelado';
}
...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ title, status, pedidos, onDragStart, onDragOver, onDrop }) => {
  return (
    <div
    ...
    >
      <h3 className="font-bold text-lg mb-4 text-gray-800">{title} ({pedidos.length})</h3>
      {pedidos.map(pedido => (
        <div
          key={pedido.id}
        ...
        >
          <p className="font-semibold text-blue-600">#{pedido.numero_pedido}</p>
          <p className="text-sm text-gray-700">{pedido.cliente_nome}</p>
          <p className="text-xs text-gray-500">Total: R$ {pedido.valor_total.toFixed(2)}</p>
        </div>
      ))}
    </div>
  );
...
const KanbanPedidos: React.FC = () => {
  ...
  const onDrop = async (e: React.DragEvent, newStatus: Pedido['situacao']) => {
    const pedidoId = parseInt(e.dataTransfer.getData('pedidoId'));
    const updatedPedidos = pedidos.map(pedido =>
      pedido.id === pedidoId ? { ...pedido, situacao: newStatus } : pedido
    );
    ...
    try {
      ...
      console.log(`Pedido ${pedidoId} atualizado para ${newStatus} no backend.`);
    } catch (e: unknown) {
      let message = "Erro desconhecido";
      if (e instanceof Error) {
        message = e.message;
  ...
  if (loading) {
    return <div className="text-center p-6">Carregando Kanban de Pedidos...</div>;
  }
    ...
    <div className="kanban-board flex space-x-4 overflow-x-auto p-6 bg-white shadow-md rounded-lg">
      {statuses.map(col => (
        <KanbanColumn
          ...
          title={col.title}
          status={col.status}
          pedidos={pedidos.filter(pedido => pedido.situacao === col.status)}
