...
const GraficoTendencias: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTendenciaData = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('http://localhost:3000/api/grafico-tendencias');
        if (!response.ok) {
  ...
  if (loading) {
    return <div className="text-center p-6">Carregando gráfico de tendências...</div>;
  }
  ...
  if (tendenciaData.length === 0) {
    return <div className="text-center p-6">Nenhum dado de tendência disponível.</div>;
  }

  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">Gráfico de Tendências Sazonais</h2>
      <div className="h-64 flex items-center justify-center bg-gray-100 rounded-md text-gray-500">
        ...
        <ul className="list-disc list-inside mt-2">
          {tendenciaData.map((item, index) => (
            <li key={index}>{item.mes}: Ano Atual - R$ {item.vendasAnoAtual.toFixed(2)}, Ano Anterior - R$ {item.vendasAnoAnterior.toFixed(2)}</li>
          ))}
        </ul>
      </div>
    </div>
