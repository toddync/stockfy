...
const HeatmapVendasGrupos: React.FC = () => {
        ...
        const fetchHeatmapData = async () => {
            ...
            try {
                const response = await fetch(
                    "http://localhost:3000/api/heatmap-vendas-grupos",
                );
                if (!response.ok) {
                ...
                setError(
                    `Falha ao carregar mapa de calor de vendas: ${message}`,
                );
    ...
    if (loading) {
        return (
            <div className="text-center p-6">Carregando mapa de calor...</div>
        );
    }
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">
                Mapa de Calor de Vendas por Grupo
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {heatmapData.map((data, index) => (
                    ...
                    >
                        ...
                        <p className="text-2xl">R$ {data.vendas}</p>
                    </div>
                ))}
            </div>
        </div>
