...
const ClassificadorABC: React.FC = () => {
        ...
        const fetchABCData = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    "http://localhost:3000/api/produtos/abc-analysis",
                );
                if (!response.ok) {
    ...
    if (loading) {
        return (
            <div className="text-center p-6">
                Carregando análise de Curva ABC...
            </div>
        );
    }
    ...
    if (abcData.length === 0) {
        return (
            <div className="text-center p-6">
                Nenhum dado para análise de Curva ABC encontrado.
            </div>
        );
    }
                        ...
                        {abcData.map((item) => (
                            ...
                            >
                                ...
                                <td className="py-3 px-4 text-right text-sm text-gray-700">
                                    R$ {item.valorTotalVendas}
                                </td>
                                <td className="py-3 px-4 text-right text-sm text-gray-700">
                                    {item.porcentagemAcumulada}%
                                </td>
