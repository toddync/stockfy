    ...
    <div className="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        {produtos.length > 0 && (
          <div className="border border-gray-300 rounded max-h-48 overflow-y-auto mt-2">
            {produtos.map(product => (
              ...
              >
                <span>{product.descricao} (R$ {product.preco_venda})</span>
      ...
      <div className="mt-6">
              ...
              <tbody>
                {carrinho.map(item => (
                  <tr key={item.produto.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-sm text-gray-700">R$ {item.produto.preco_venda}</td>
                    ...
                    <td className="py-3 px-4 text-sm text-gray-700">R$ {(item.produto.preco_venda * item.quantidade)}</td>
                    <td className="py-3 px-4 text-sm text-gray-700">R$ {((item.produto.preco_venda * item.quantidade) * (item.comissaoPercentual / 100))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-md text-right">
          <p className="text-lg font-bold text-gray-800">Total Venda: R$ {calculateTotalVenda()}</p>
          <p className="text-xl font-bold text-blue-600">Comissão Total: R$ {calculateTotalComissao()}</p>
          {selectedVendedor && (
