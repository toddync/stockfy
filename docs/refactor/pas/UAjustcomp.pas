...
function TFrmAjustcomp.ExisteForm(pForm: Tform): Boolean;
var
  I: Integer;
begin
  ExisteForm := False;
...
procedure TFrmAjustcomp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmAjustcomp.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAjustcomp.FormShow(Sender: TObject);
begin
  chvoper:='I';
  edit1.Clear;
  edit3.Clear;
...
procedure TFrmAjustcomp.ComboBox1Exit(Sender: TObject);
begin
...
procedure TFrmAjustcomp.ComboBox2Exit(Sender: TObject);
begin
...
procedure TFrmAjustcomp.ComboBox4Exit(Sender: TObject);
begin
...
procedure TFrmAjustcomp.Edit10Exit(Sender: TObject);
begin
  xparcela:=0;
  zparcela:=0;
  if edit10.text<' ' then
...
procedure TFrmAjustcomp.Edit15Exit(Sender: TObject);
begin
...
procedure TFrmAjustcomp.Edit16Exit(Sender: TObject);
begin
  // Atualiza Operacao ***********************
  if chvachou='E' then
  begin
  ...
  begin
    ...
    //aqtda:=IBQAuxitens.fieldbyName('Qtdant').AsFloat;
...
procedure TFrmAjustcomp.Edit17Exit(Sender: TObject);
begin
...
procedure TFrmAjustcomp.Edit18Exit(Sender: TObject);
begin
  G:=edit18.text;
  Richedit2.Lines.Add('  FRETE           '+StringofChar(#32,15-Length(G))+(G));
end;

procedure TFrmAjustcomp.Edit19Exit(Sender: TObject);
begin
  G:=edit19.text;
...
procedure TFrmAjustcomp.Edit2Exit(Sender: TObject);
begin
  if edit2.text='' then
  begin
...
procedure TFrmAjustcomp.Edit3Exit(Sender: TObject);
begin
  if edit3.Text<>'' then
  begin
    While Length(edit3.text)<2 do
...
procedure TFrmAjustcomp.Edit4Exit(Sender: TObject);
begin
  if edit4.Text='' then
  begin
  ...
  begin
    ...
    if wsitu<>'R' then
    begin
      showmessage('Pedido não foi recebido  !!!!');
      Limpando;
      exit;
    end;
    if MessageDlg('Pedido encontrado. Deseja alterar ???',
...
procedure TFrmAjustcomp.Edit6Exit(Sender: TObject);
begin
  While Length(edit6.text)<8 do
        edit6.text:='0'+edit6.text;
end;

procedure TFrmAjustcomp.Edit7Exit(Sender: TObject);
begin
  if edit7.text='' then
  begin
    Combobox2.SetFocus;
...
procedure TFrmAjustcomp.Edit9Exit(Sender: TObject);
begin
  // Preencher a chave com zeros a esquerda
...
procedure TFrmAjustcomp.Estokar;
begin
  chvtem:='';
...
procedure TFrmAjustcomp.Achar_item;
begin
  witens:='';
  xitens:=0;
  if IBQAuxitens.Active then
...
procedure TFrmAjustcomp.Limpando;
begin
  edit1.Clear;
  edit2.Clear;
  edit3.Clear;
...
procedure TFrmAjustcomp.Cob_imp;
begin
...
procedure TFrmAjustcomp.MaskEdit1Exit(Sender: TObject);
begin
   if Combobox1.text='FAB' then
     maskedit2.Text:=maskedit1.Text;
end;

procedure TFrmAjustcomp.List_cob;
begin
...
// *********************************************************************
procedure TFrmAjustcomp.Fecha_nota;
begin
  wtotnot:=0;
  IBQAuxitens.Close;
  IBQAuxitens.SQL.Clear;
  ...
  // Limpas  conta a pagar anterior *****
  DModRosa.IBQVer_cont.Active:=False;
  DModRosa.IBQVer_cont.Sql.Clear;
  DModRosa.IBQVer_cont.SqL.Add('Delete from CONTAS '+
                               'Where TIPDOC=:tip_digit '+
                               'and NUMDOC=:num_digit');
  DModRosa.IBQVer_cont.ParamByName('tip_digit').AsString:=ComboBox1.Text;
  DModRosa.IBQVer_cont.ParamByName('num_digit').AsString:=edit4.Text;
  DModRosa.IBQVer_cont.Active:=True;
  DModRosa.IBQVer_cont.Close;
  // Cria Novas Contas a pagar *******************
  if IBQAuxcont.Active then
     IBQAuxcont.Active:=False;
  IBQAuxcont.Sql.Clear;
  IBQAuxcont.Sql.Add('Select * from AUXCONTA '+
...
procedure TFrmAjustcomp.Achar_num;
begin
  wnumped:='';
  xnumped:=0;
